...
void generateSphere(float radius, int divisions) {
	for (int i = 0; i <= 2 * divisions; i++) {
		for (int j = divisions / 2; j >= -divisions / 2; j--) {
			float phy = glm::radians((float)i * 360 / 2 / divisions);
			float theta = glm::radians((float)j * 180 / divisions);
			Vertex v;
			...
			v.texCoord = glm::vec2(((float)i) / divisions, ((float)-j + divisions / 2) / divisions);
