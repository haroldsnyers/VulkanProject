...
void generateRope(float radius) {

	// vertexline is the number of vertices you want your rope to have
	int vertexline = 3;
	for (int i = 0; i < vertexline + 1; i++) {
		float x = 0;
		float z = 0;
		float y = i * radius;

		Vertex v;
		// x: out or in, y: left or right, z: up or down
		v.pos = glm::vec3(x, y, -z);
		// speed set to zero otherwise, rope moves out of image
		v.speed = glm::vec3(0);
		v.color = glm::vec3(255, 50, 25);
		// first vertex moint doesnt move
		if (i == 0) {
			v.movable = 0.0f;
		}
		else {
	...
	// connection of the different verteces
...
private:
	...
	void updateVertices() {
		...
		for (int i = 0; i < vertices.size(); i++) {
			...
			vertexData[i].speed *= 0.99999999999;
